...
public class SMTSolver {
    ...
    public void initialize() throws Z3Exception {
        ...
        encodeProgram();
        lastRank = new int[program.getSize()];
        updateLastRank();
    }
    ...
    void initActs() {
        ...
        for (Process process : program.getAllProcesses()) {
            for (Operation operation : process.ops) {
                if (process.ops.indexOf(operation) < candidate.tracker[process.rank]){
                    acts.add(operation);
                }
            }
        }
        ...
        for(Operation operation : acts){
            if(program.matchOrderTables.containsKey(operation)) {//a <mo b
                for (Operation succOp : program.matchOrderTables.get(operation)) {
                    succOp = ((NewProgram) program).getCsecOp(succOp);
                    if (succOp.rank < lastRank[succOp.proc]) {
                ...
                if(operation.rank>0){
                    if(operation.isSend()){
                        ...
                        if(rank>1){
                            ...
                            if (program instanceof NewProgram)predOp = ((NewProgram) program) .getCsecOp(predOp);
            ...
            if(operation.isWait()){
                if(operation.req.isRecv()){
                    ...
                    for(Operation matchOp : matchTable){
                        matchOp = ((NewProgram)program).getCsecOp(matchOp);
...
//        if(b==null) System.out.println("[ERROR] mkSendMatch: RETURNS NULL!");
    ...
    void updateLastRank(){
        for (int i = 0; i < candidate.tracker.length; i++) {
            lastRank[i] = program.get(i).ops.get(candidate.tracker[i]).rank;
        }
    }
